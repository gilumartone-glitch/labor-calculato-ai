...
const LaborUsageSection = ({ project, updateProject }: { project: WoodProject; updateProject: (patch: Partial<WoodProject>) => void }) => (
  <Card className="border-2 border-dept shadow-soft">
    <CardHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between"><CardTitle>Squadra montaggio e ore</CardTitle><Button size="sm" onClick={() => updateProject({ labor: [...project.labor, { id: uid(), workerId: project.workers[0]?.id ?? "", hours: 1 }] })}><Plus className="h-4 w-4" />Riga</Button></CardHeader>
    <CardContent className="space-y-3">
      {project.labor.map((line) => {
        ...
        return <div key={line.id} className="rounded-sm border border-border bg-background p-3">
          ...
          <Field label="Totale"><div className="flex h-10 items-center font-mono font-semibold">{eur(worker ? (workerHourlyCost(worker) + (line.travel ? 2.5 : 0)) * line.hours : 0)}</div></Field>
